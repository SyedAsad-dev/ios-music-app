...
class RealtimeAnalyzer {
    ...
    public var frequencyBands: Int = 80 //Number of frequency bands
    public var startFrequency: Float = 100 //Start frequency
    public var endFrequency: Float = 16000 //Cut-off frequency
    
    private lazy var bands: [(lowerFrequency: Float, upperFrequency: Float)] = {
        var bands = [(lowerFrequency: Float, upperFrequency: Float)]()
        //1：Determine the multiple of the increase according to the start and end frequency spectrum and the number of frequency bands: 2^n
        ...
        for i in 1...frequencyBands {
            //2：The upper frequency point of the frequency band is 2^n times the lower frequency point
    ...
    private func fft(_ buffer: AVAudioPCMBuffer) -> [[Float]] {
        ...
        //1：Extract sample data in buffer
        ...
        for i in 0..<channelCount {
            ...
            //3: Pack the real number into the complex number fftInOut required by FFT, which is both input and output
    ...
    private func highlightWaveform(spectrum: [Float]) -> [Float] {
        //1: Define the weight array, the 5 in the middle of the array represents your own weight
        // Can be modified at will, the number needs to be odd
